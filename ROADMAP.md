...
| **Type checking that fails** | Planned | **The highest-priority item in this file.** Today an unknown identifier types as `Int` and compiles. Making it an error is a small change with the largest downstream effect, and it will surface real bugs in existing `.waa` files. Prerequisite for every item in Phases 3-5. |
