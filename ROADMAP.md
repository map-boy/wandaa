...
| **Bitwise operations** | **Done** | `biti_na`, `biti_cyangwa`, `biti_gutandukana`, `biti_ibumoso`, `biti_iburyo` — AND, OR, XOR and the two shifts, over all 64 bits. Builtins rather than operators for one concrete reason: `>>` cannot be lexed as a single token without breaking `urutonde<igisubizo<umubare>>`, where the two closing angles are two tokens. Needed by anything touching a binary format, and by the code generator the bootstrap still has to grow. The five encodings behind them (`and`, `or`, `shl`/`shr`/`sar` by CL) are diffed against GNU `as` across all 16 registers like everything else. |
