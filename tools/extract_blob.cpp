...
int main(int argc, char** argv) {
    ...
    // ---- the same tables, as plain text for the Wandaa codegen ----------
    // include/runtime_blob.hpp is a C++ header; compiler/codegen.waa cannot
    // read one. The same data goes out as text beside it, so both compilers
    // work from a single extraction rather than two that could drift.
    {
        const std::string txtPath =
            outPath.size() > 4 && outPath.substr(outPath.size()-4) == ".hpp"
            ? outPath.substr(0, outPath.size()-4) + ".txt" : outPath + ".txt";
        std::ofstream t(txtPath);
        if (!t) fail("cannot write " + txtPath);
        t << "# Generated by tools/extract_blob.cpp -- do not edit.\n";
        t << "blob ";
        for (size_t i = 0; i < text.size(); ++i) {
            char buf[4];
            std::snprintf(buf, sizeof buf, "%02x", text[i]);
            t << buf;
        }
        t << "\n";
        for (const auto& x : impFixups)
            t << "import " << x.site << " " << x.dll << " " << x.func << " " << x.addend << "\n";
        for (const auto& x : codeFixups)
            t << "code " << x.site << " " << x.sym << " " << x.addend << "\n";
        for (const auto& kv : labels)
            t << "label " << kv.first << " " << kv.second << "\n";
    }
